...
##Escreva uma função chamada **compara** que receba como parâmetro duas listas duplamente ligadas. 
##Ela deve retornar True caso as duas listas duplamente ligas passadas como parâmetro sejam iguais e False caso
##contrário. Duas listas são consideradas iguais tanto quando os elementos estão na mesma ordem ou em ordem contrária. 
##Exemplo: A = [0,1,2,3,4], B = [0,1,2,3,4] e C = [4,3,2,1,0] são listas iguais.
...
def inverte(lista, noh = -1, novaLista = None):  
    ...
    if noh==None:
        ...
        novaLista = inverte( lista, noh.next, novaLista )
        
        if noh.data%2 == 1:
            novaLista.insertTail(noh.data)
        
        return novaLista
        
  
        
def inverte(lista, node = -1, lista_invertida = None):
  """
  ...
  lista_invertida.insertTail(node.data)
  ...
  lista_invertida = inverte(lista, node.prev, lista_invertida)
